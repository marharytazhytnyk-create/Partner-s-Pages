...
Структура: MBR Bella Mozzarella / Pinkman Bar (бар-чарти, KPI-картки, розкладні локації, аналіз).
Автооновлення: 3-го числа кожного місяця о 13:00 (Київ) через GitHub Actions.
"""
...
OUTPUT_HTML     = SCRIPT_DIR / "MBR Vertel.html"
POLL_INTERVAL_S = 5
MAX_POLL_S      = 600
CLUSTER_START_S = 900
FETCH_ATTEMPTS  = 3
...
# Замовлень на одного активного гостя за місяць. Реальне значення — близько 1.2—1.5;
# межа стоїть далеко вище, щоб ловити лише зіпсовані дані, а не вдалий місяць.
MAX_PLAUSIBLE_FREQ = 10.0
...
BRANDS_CONFIG = [
    {
        ...
        "emoji":        "🌯",
        "color":        "#E65100",
        "provider_ids": [182418, 182421, 182423, 182428, 185858, 194235],
        "name_strip":   r"Вертел\s*(?:\(МАФ\))?\s*\.?\s*",
    },
]
...
MONTH_BAR_COLORS_VERTEL = [
    "#7f2704","#a63603","#bf4904","#d94801",
...
def pick_schema(ctx: str) -> str:
    """Усі таблиці цього звіту мають бути в одній схемі; інакше падаємо на hive_metastore."""
    global SCHEMA
    if SCHEMA:
        return SCHEMA
    for schema in SCHEMA_CANDIDATES:
        try:
            ...
            continue
    raise RuntimeError(
        "Не знайдено схему, де читаються dim_provider_v2, "
        "fact_provider_monthly і int_provider_metrics_non_additive"
    )
...
def fetch_brand_data(brand: dict) -> dict:
    ...
    # Build location dict
    loc_map = {int(r[0]): {"name": str(r[1]), "city": CITY, "zone": str(r[3] or "")}
               for r in loc_rows}
    ...
    for pid in sorted(by_pid.keys(), key=lambda x: loc_map.get(x, {}).get("name", "")):
        info = loc_map.get(pid, {"name": f"ID {pid}", "city": CITY, "zone": ""})
        months_data = []
...
def assert_plausible(data: dict) -> None:
    """Reject months where orders far outrun the number of guests who placed them.

    In August 2026 the Chernihiv order counts arrived multiplied by 32 while the
    unique-user counts stayed correct, so a corrupted month shows an impossible
    number of orders per guest. Publishing that would tell the partner they sold
    thirty times more than they did.
    """
    for m in data["brand_months"]:
        users, orders = m.get("active_users", 0), m.get("orders", 0)
        if users and orders / users > MAX_PLAUSIBLE_FREQ:
            raise RuntimeError(
                f"{m['label']}: {orders} замовлень на {users} унікальних гостей "
                f"({orders / users:.1f} зам./гість) — дані виглядають зіпсованими")


def fetch_brand_data_checked(brand: dict) -> dict:
    """Retry the fetch and reject empty or implausible results.
...
def build_brand_panel(brand: dict, data: dict, bar_colors: list) -> str:
    ...
    <div class="period-bar">
      ...
      <span>{data['period_label']} &nbsp;·&nbsp; валюта UAH (₴) &nbsp;·&nbsp; {CITY}</span>
...
def build_html(brands_data: list[tuple[dict, dict]]) -> str:
    ...
    for i, (brand, data) in enumerate(brands_data):
        ...
        panel_html = build_brand_panel(brand, data, MONTH_BAR_COLORS_VERTEL)
        brand_tabs += (
...
<header class="header">
  <div class="header-left">
    ...
    <div class="header-title">
      <h1>MBR · VERTEL</h1>
      <p>Bolt Food &nbsp;·&nbsp; Місячний звіт &nbsp;·&nbsp; {CITY}</p>
    </div>
  </div>
...
<div class="footer">
  Автоматично оновлюється 3-го числа кожного місяця &nbsp;·&nbsp;
  <span>Bolt Food MBR</span> &nbsp;·&nbsp; {CITY}
</div>
